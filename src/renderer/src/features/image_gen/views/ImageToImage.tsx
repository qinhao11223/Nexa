...
export default function ImageToImage({ onSwitchMode }: { onSwitchMode: (mode: ImageGenMode) => void }) {
  ...
  const [autoStackEnabled, setAutoStackEnabled] = useState(() => {
    try {
      const raw = localStorage.getItem(CANVAS_UI_KEY)
  ...
  const [autoStackNameMap, setAutoStackNameMap] = useState<Record<string, string>>(() => {
    try {
      const raw = localStorage.getItem(AUTO_STACK_NAME_KEY)
  ...
  useEffect(() => {
    try {
      localStorage.setItem(CANVAS_UI_KEY, JSON.stringify({ autoStackEnabled, hideNameEnabled, openGroupKey }))
    } catch {
      // 忽略
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem(AUTO_STACK_NAME_KEY, JSON.stringify(autoStackNameMap))
    } catch {
      // 忽略
    }
  }, [autoStackNameMap])
  ...
  const manualLayoutInfoForAuto = useMemo(() => {
    try {
      const raw = localStorage.getItem(MANUAL_LAYOUT_KEY)
  ...
  const moveTasksIntoManualFolder = (folderId: string, taskIds: string[]) => {
    ...
    try {
      ...
      localStorage.setItem(MANUAL_LAYOUT_KEY, JSON.stringify({ ...parsed, root: root2, folders: foldersObj }))
      // 触发重新计算“自定义文件夹/未分类”
      setManualRefreshToken(v => v + 1)
    } catch {
      // 忽略
